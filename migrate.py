...
# Configuration
MAX_RETRIES = 3
IGNORABLE_SQLITE_FOREIGN_KEY_VIOLATIONS = {
    ("chat_file", "chat"),
...
def classify_sqlite_foreign_key_violations(
    ...
    """Split Open WebUI orphan join rows from unknown FK violations."""
    skipped_rowids: Dict[str, List[int]] = {}
    unknown_violations = []

    for violation in violations:
        table_name, rowid, parent_table, _fk_id = violation
        if (
            rowid is not None
            and (table_name, parent_table) in IGNORABLE_SQLITE_FOREIGN_KEY_VIOLATIONS
        ):
            skipped_rowids.setdefault(table_name, []).append(rowid)
        else:
            unknown_violations.append(violation)

    return skipped_rowids, unknown_violations


def build_sqlite_rowid_skip_clause(rowids: List[int]) -> Tuple[str, Tuple[int, ...]]:
    if not rowids:
        return "", ()

    placeholders = ", ".join("?" for _ in rowids)
    return f" WHERE rowid NOT IN ({placeholders})", tuple(rowids)
    ...
    while True:
        ...
        if not tables_exist:
            ...
            if not Confirm.ask(
                ...
                console.print(
                    "[red]Migration cancelled. Please bootstrap PostgreSQL database first.[/]"
                )
                sys.exit(0)
            console.print("\n")  # Add spacing before retry
            continue
...
def get_sqlite_integrity_report(db_path: Path) -> SQLiteIntegrityReport:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            cursor = conn.cursor()
            skipped_foreign_key_rowids: Dict[str, List[int]] = {}
            ...
            for check_name, query in checks:
                ...
                if check_name == "Foreign Key Check" and result:
                    skipped_rowids, unknown_violations = (
                    ...
                    skipped_foreign_key_rowids = skipped_rowids
                    total_skipped = sum(
...
async def process_table(
    ...
    try:
        ...
        # Process rows
        if skipped_sqlite_rowids:
            console.print(
                f"[yellow]Skipping {len(skipped_sqlite_rowids)} orphaned row(s) "
                f"from {table_name}[/]"
            )

        sqlite_cursor.execute(
            f"SELECT COUNT(*) FROM {sqlite_safe_table_name}{skip_clause}",
            skip_params,
        )
        ...
        while processed_rows < total_rows:
            try:
                sqlite_cursor.execute(
                    f"SELECT * FROM {sqlite_safe_table_name}{skip_clause} "
                    "LIMIT ? OFFSET ?",
                    (*skip_params, batch_size, processed_rows),
                )
...
async def migrate() -> None:
    ...
    integrity_report = get_sqlite_integrity_report(sqlite_path)
